...
| **Freshness** | Read access to action-relevant current GitHub state plus an atomic binding or equivalent isolation mechanism for every mutable identity that is decision-critical at consequence time | Immediate pre-action re-fetch of candidate, base/dependencies, assurance, review, authority, target and material blockers, plus deterministic materiality classification and actuation-time atomic binding or equivalent isolation of each decision-critical mutable identity | Any decision-critical fact is stale, ambiguous, partial, contradictory or inaccessible, materiality is unknown, or a decision-critical mutable identity cannot be atomically bound or equivalently isolated across actuation | Dismiss-stale-review policies, required up-to-date branch, merge queue |
| **Consequence Authority** | Durable owner/human GitHub-native record, current-state reads and atomic conditional or equivalent isolation capability for every mutable identity that remains decision-critical at consequence time | Exact repository, governing contract, exact candidate/state, named consequence, prospective human authority, immediate requalification and actuation-time atomic binding or equivalent isolation of each decision-critical mutable identity; explicit evidence IDs when accepted evidence is ambiguous | Authority names old state/consequence, accepted evidence is ambiguous, currentness cannot be proved, materiality is unknown, or any decision-critical mutable identity cannot be atomically bound or equivalently isolated across actuation | Protected branches, required reviews/checks, environment approval for separately governed deployment consequences |
...
Pre-consequence requalification and consequence actuation form one correctness boundary. A last-moment re-read establishes only the state observed at that instant; it does not by itself close a race across actuation. Every mutable identity that is decision-critical at consequence time must be atomically bound or equivalently isolated across actuation. If neither can be established, consequence actuation fails closed.

Candidate-head change always requires requalification of affected assurance/review/currentness. For other mutable target or base state, movement is decision-critical when it can materially affect correctness, validation assumptions, dependencies, security/authority, conflict resolution or resulting consequence identity. A mutable identity may remain unbound only when deterministic evidence establishes that any movement during actuation is non-decision-critical under that materiality rule. A pre-actuation snapshot alone cannot establish that property. Unknown materiality, or materially decision-critical state that cannot be atomically bound or equivalently isolated, fails closed before consequence.
...
For every mutable identity that is decision-critical at consequence time, a conforming path must atomically bind or equivalently isolate that identity across actuation. An exposed expected-state precondition must be used when it provides that binding. If decision-critical mutable state cannot be atomically bound or equivalently isolated, consequence actuation fails closed. This is part of the minimum private-Free execution mechanism, not optional paid enforcement.

For an ordinary GitHub pull-request merge, expected-head guarded merge is mandatory: bind the merge request to the authorised exact head SHA using GitHub's merge `sha` precondition or the connected capability's `expected_head_sha`. If the PR head moved, the merge must be rejected before consequence. Expected-head guarding binds the PR head only; by itself it does not bind materially decision-critical base or target state. It is sufficient only when base/target movement is deterministically non-decision-critical at consequence time, or when an additional mechanism atomically binds or equivalently isolates every materially decision-critical base/target identity across actuation. If neither condition is established, the merge fails closed before actuation; a last-moment base/target re-read alone is not sufficient. When GitHub positively proves that the expected-head precondition rejected actuation before the merge boundary, that is a positively proven pre-actuation/no-effect failure under Proof A's existing authority-consumption rules; it is not a possibly-effective consequence merely because a merge call was attempted.
...
- [ ] Every mutable identity that is decision-critical at consequence time is atomically bound or equivalently isolated across actuation; if neither can be established, consequence actuation fails closed.
- [ ] Ordinary GitHub PR merge uses mandatory expected-head guarded actuation, treats that guard as sufficient only when base/target movement is deterministically non-decision-critical or separately atomically bound/equivalently isolated, and never treats a last-moment re-read alone as closing the race; a positively proven expected-head mismatch fails closed before merge without being classified as a possibly-effective consequence.
...
Control: human implementation and consequence authority remain durable semantic predicates. The conforming executor revalidates exact current state before actuation, then atomically binds or equivalently isolates every mutable identity that remains decision-critical at consequence time. If that protection cannot be established, actuation stops rather than relying on the last observed state.

### Risk: state moves between requalification and consequence actuation

Control: pre-consequence requalification alone is not treated as sufficient. Every decision-critical mutable identity must be atomically bound or equivalently isolated across actuation, and the path fails closed when that cannot be established. For ordinary GitHub PR merge, expected-head guarding is mandatory and rejects a moved head without paid enforcement, but it is sufficient only when base/target movement is deterministically non-decision-critical or separately atomically bound/equivalently isolated; a last-moment base/target read does not close that race.
